...
import {APP_SRC, APP_ASSETS, TMP_DIR, CSS_DEST, APP_DEST, BROWSER_LIST, ENV} from '../../config';
...
function processExternalScss() {
  return gulp.src(getExternalScss().map((r:any) => r.src))
...
function getExternalScss() {
  return APP_ASSETS.filter((d:any) => /\.scss$/.test(d.src));
}
